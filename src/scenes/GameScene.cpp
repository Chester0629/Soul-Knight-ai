...
void GameScene::OnEnter() {
    ...
    // Hero id this floor (floor 0: picked in HeroSelect; later floors: carried).
    // Determined BEFORE the player is built so the in-game sprite matches the pick.
    m_CharId = m_Carried ? m_Carried->charId
                         : (m_Run != nullptr ? m_Run->State().selectedCharId
                                             : std::string("c01"));
    m_Player = std::make_shared<Player>(m_Data.PlayerTemplate(), root, m_CharId);
    ...
    for (int i = 0; i < 6; ++i) {
        floorImgs[static_cast<std::size_t>(i)] = std::make_shared<Util::Image>(
            root + "/sprites/p_floor" + std::to_string(i + 1) + ".png");
    }
    ...
    for (const RoomCell &cell : floor.Rooms()) {
        ...
        if (roomIndex != floor.StartIndex() && !spawnCells.empty()) {
            ...
            if (roomIndex == bossRoom && IsBossFloor(m_FloorIndex)) {
                ...
                // Per-type boss sprite (G8 re-skin); boss12 has no art -> boss01.
                std::string bossSprite = "boss01";
                if (std::string(bossId) == "BossAI06") {
                    bossSprite = "boss06";
    ...
    // m_CharId was resolved above (before the player build); reuse it here.
